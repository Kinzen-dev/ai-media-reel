...
- Batch 27+ FUEL is now huge: `_backlog-refresh-2.json` (34 ready) + the older `_backlog-refresh.json` leftovers (voice-transcript, fixed-viewport, live-reaction-broadcast, fashion-editorial, semantic-zoom, culture-capsule, chaptered-deck, brutalist-utility, seriograph, viewfinder-ar + 2 WebGL). PICK DIVERSE batches (one per axis). King pre-authorized: keep the loop running through these without waiting for confirm. Proposed batch 27 (diverse, all strength-5, capture-safe): swiss-international-typographic, de-stijl-mondrian, api-docs-dark-sidebar, css-voxel-isometric, fintech-trust-stack.
