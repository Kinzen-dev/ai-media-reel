...
- Batch 25 IN FLIGHT (wgrq98ur2): cmd-k-portfolio, annotated-case-study, stat-annual-report, changelog-diff-timeline, frutiger-aero-revival.
- Batch 26+ fuel (in `_backlog-refresh.json`, NOT built): voice-transcript, split-master-detail, radial-hub, fixed-viewport, live-reaction-broadcast, broadsheet-newspaper, fashion-editorial, silent-luxury, semantic-zoom, culture-capsule, chaptered-deck, la-carta-restaurant, brutalist-utility, seriograph, viewfinder-ar + 2 WebGL (cinematic-single-room, terrain-map-navigation). King pre-authorized: keep the loop running through these without waiting for confirm. After batch 25 (~15/32 built = HALFWAY), run another BROAD multi-lens audit (not a single-axis refresh) per the research-breadth rule.
