...
- Batch 23 DONE + cataloged (5 new page-purpose types, all PASS first time -> 116 live). Batch 24 IN FLIGHT: linkinbio-glass-stack, recipe-howto-card, interactive-timeline, multistep-onboarding-form, token-stream-canvas.
- Batch 25+ fuel (in `_backlog-refresh.json`, NOT built): voice-transcript, cmd-k-portfolio, split-master-detail, radial-hub, fixed-viewport, annotated-case-study, live-reaction-broadcast, stat-annual-report, broadsheet-newspaper, fashion-editorial, silent-luxury, frutiger-aero, changelog, semantic-zoom, culture-capsule, chaptered-deck, la-carta-restaurant, brutalist-utility, seriograph, viewfinder-ar + 2 WebGL (cinematic-single-room, terrain-map-navigation). King pre-authorized: keep the loop running through these without waiting for confirm. When this 32-idea backlog is ~half built, run another BROAD multi-lens audit (not a single-axis refresh) per the research-breadth rule.
